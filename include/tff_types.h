...
struct TimeVal {
    ...
    int64_t toMicros() const { return sec * 1000000LL + usec; }

    static TimeVal fromMicros(int64_t us) { return TimeVal{us / 1000000LL, us % 1000000LL}; }

    static TimeVal maxTime() { return TimeVal{std::numeric_limits<int64_t>::max() / 2, 999999}; }
    ...
    bool operator>(const TimeVal& o) const { return o < *this; }

    bool operator==(const TimeVal& o) const { return sec == o.sec && usec == o.usec; }

    bool operator!=(const TimeVal& o) const { return !(*this == o); }
...
enum KeyValue : int32_t { KEY_VAL_UP = 0, KEY_VAL_DOWN = 1, KEY_VAL_REPEAT = 2 };

enum EventType : uint16_t { EV_SYN = 0x00, EV_KEY = 0x01, EV_REL = 0x02, EV_MSC = 0x04 };

enum SynCode : uint16_t { SYN_REPORT = 0 };

enum RelCode : uint16_t { REL_X = 0x00, REL_Y = 0x01, REL_HWHEEL = 0x06, REL_WHEEL = 0x08 };

enum MscCode : uint16_t { MSC_SCAN = 4 };
...
struct MouseAction {
    ...
    bool isRelative() const { return isMovement() || isWheel(); }

    bool operator==(const MouseAction& o) const { return type == o.type && delta == o.delta; }
};
...
struct Layer {
    ...
    bool operator==(const Layer& o) const { return name == o.name && mappings == o.mappings; }
};
...
struct TapHoldKey {
    ...
    int64_t tap_one_shot_timeout_us =
        1500000LL;                 // One-shot expiration timeout in us (default 1500ms)
    ...
    bool operator==(const TapHoldKey& o) const {
        return key == o.key && tap_key == o.tap_key && hold_key == o.hold_key &&
               ...
               tap_one_shot_timeout_us == o.tap_one_shot_timeout_us && tap_leader == o.tap_leader &&
...
struct OneShotKey {
    ...
    bool operator==(const OneShotKey& o) const {
        return key == o.key && modifier == o.modifier && layer == o.layer &&
